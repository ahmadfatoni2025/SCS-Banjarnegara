...
// Jika kosong, arahkan ke home
if ($path === '') {
    $path = 'dashboard'; // atau 'home' sesuai kebutuhan
}

// List halaman yang diperbolehkan
$pages = [
    'dashboard',
    'login',
    'profile',
...
if (in_array($path, $pages) && file_exists($path . '.php')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['hapus_suplier_id'])) {
    ...
    if ($id_sup_del > 0) {
        try {
            ...
            $stmt_upd = $koneksi->prepare("UPDATE gudang SET id_suplier = NULL WHERE id_suplier = ?");
            $stmt_upd->bind_param("i", $id_sup_del);
            $stmt_upd->execute();
            $stmt_upd->close();
            
            // 2. Hapus suplier dari database
            $stmt_del = $koneksi->prepare("DELETE FROM data_suplier WHERE id_suplier = ?");
            $stmt_del->bind_param("i", $id_sup_del);
            if ($stmt_del->execute()) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] !== 'hapus_suplier') { // Cek agar tidak bentrok
    ...
    $stok = (float)($_POST['stok'] ?? 0); // Ubah ke float untuk mendukung desimal (misal 1.5 kg)
    ...
    if ($_POST['action'] === 'tambah') {
        try {
            $stmt = $koneksi->prepare("INSERT INTO gudang (nama, keterangan, kategori, satuan, harga_beli, harga, stok, id_suplier) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssdddi", $nama, $keterangan, $kategori, $satuan, $harga_beli, $harga, $stok, $id_suplier); // dddi karena stok bisa float
        ...
        try {
            $stmt = $koneksi->prepare("UPDATE gudang SET nama = ?, keterangan = ?, kategori = ?, satuan = ?, harga_beli = ?, harga = ?, stok = ?, id_suplier = ? WHERE id_barang = ?");
            $stmt->bind_param("ssssdddii", $nama, $keterangan, $kategori, $satuan, $harga_beli, $harga, $stok, $id_suplier, $id_barang); // dddii
...
if (isset($_GET['action'])) {
    if ($_GET['action'] === 'tambah') {
        ...
        $id_barang = (int)$_GET['id'];
        $sql_edit = "SELECT g.id_barang, g.nama, g.keterangan, g.kategori, g.satuan, g.harga_beli, g.harga, g.stok, g.id_suplier, s.nama_suplier 
                     FROM gudang g 
        ...
        if ($stmt_edit->num_rows > 0) {
            $stmt_edit->bind_result($id_barang_res, $nama_res, $ket_res, $kategori_res, $satuan_res, $harga_beli_res, $harga_res, $stok_res, $id_suplier_res, $nama_suplier_res);
            $stmt_edit->fetch();
            $data_edit = [
...
$inventory_data = [];
$sql_select = "SELECT g.id_barang, g.nama, g.keterangan, g.kategori, g.satuan, g.harga_beli, g.harga, g.stok, g.is_pinned, s.nama_suplier 
               FROM gudang g 
               LEFT JOIN data_suplier s ON g.id_suplier = s.id_suplier 
               $sql_where 
...
if ($stmt_select) {
    ...
    $stmt_select->execute();
    $stmt_select->bind_result($id_barang_res, $nama_res, $ket_res, $kategori_res, $satuan_res, $harga_beli_res, $harga_res, $stok_res, $is_pinned_res, $nama_suplier_res);
                    ...
                    <table class="min-w-full divide-y divide-gray-200 compact-table sticky-header">
                        ...
                        <tbody id="product-table-body" class="bg-white divide-y divide-gray-100">
                            ...
                            <tr class="<?= $status['row_class'] ?> product-row group hover:bg-gray-50 transition-colors" data-nama="<?php echo htmlspecialchars(strtolower($product['nama'])); ?>">
                                ...
                                <td class="text-right font-mono font-bold text-base <?= $status['text_class'] ?>"><?php echo $product['stok']; ?></td>
        ...
        <div class="modal-content bg-white rounded-xl shadow-2xl max-w-2xl w-full mx-4 overflow-hidden border border-gray-100 flex flex-col">
            ...
            <form id="product-form" method="POST" action="index.php" class="p-6 overflow-y-auto flex-1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-5">
                    ...
                    <div class="md:col-span-2"><label class="block text-sm font-medium text-gray-700 mb-1.5">Stok</label><input type="number" step="any" name="stok" required class="w-full border border-gray-300 p-2.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-center font-semibold" value="<?php echo $data_edit['stok'] ?? '0'; ?>"></div>
